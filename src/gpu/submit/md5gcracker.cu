...
/* http://stackoverflow.com/questions/13245258/
   handle-error-not-found-error-in-cuda */
...
/* kernel */
__global__ void cudaCrack(md5_byte_t *hash_digest, Word *words,
                          int num_words, int *result) {
...
void crackHash(char* hash, int dict) {
   ...
   while (words_done < numWords) {
       ...
       HANDLE_ERROR(cudaMemcpy(cu_words, words + words_done,
                               sizeof(Word) * words_per_iteration,
                               cudaMemcpyHostToDevice));

       cudaCrack<<<grid, block>>>(cu_hash_digest, cu_words,
                                  words_per_iteration, cu_result);

       HANDLE_ERROR(cudaMemcpy(&result, cu_result, sizeof(int),
                               cudaMemcpyDeviceToHost));
       
       if (result >= 0) {
           printf("%s -> %s\n", hash, words[result+words_done].word);
           return;
       }
...
void initWords(Word* words, char* dictFile, int numWords, int size) {
   ...
   for (i = 0; i < size; i++) {
      if (dictFile[i] == '\n') {
         ...
         strncpy(words[curWord].word, &(dictFile[i]) - wordLen + 1,
                 wordLen - 1);
         curWord++;
         wordLen = 0;
      }
